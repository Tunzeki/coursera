...
-- Task 1: Lucky Shrub need to find out how many sycamore trees they’ve sold over the past few years. 
...
-- 2020, 2021 and 2022. 
WITH 
P4_Products_Sold AS (SELECT 
    SUM(Quantity) AS Quantity, YEAR(Date) AS `Year`
    FROM Orders 
    WHERE ProductID = "P4" 
    GROUP BY YEAR(Date))
SELECT CONCAT(Quantity, " (", Year, ")") AS `P4 product: Quantity sold` FROM P4_Products_Sold ORDER BY Year;

-- Task 2: Lucky Shrub need information on all their clients and the orders that they placed in the years 2022 and 2021. 
-- See if you can help them by extracting the required information from each of the following tables:
...
-- The result set should be ordered by the “orders date” column

SELECT 
    Clients.ClientID, ContactNumber, Street, County, OrderID, Products.ProductID, ProductName, Cost, Date
FROM Clients
...
-- Task 3: Lucky Shrub needs to analyze the sales performance of their Patio slates product in the year 2021. 
-- This product has a Product ID of P3. Help Lucky Shrub to analyze the performance of this product 
-- by developing a function called “FindSoldQuantity” that enables them to:
...
CREATE FUNCTION FindSoldQuantity (product_id VARCHAR(10), year INT)
RETURNS INT DETERMINISTIC
RETURN (
    SELECT SUM(Quantity) FROM Orders WHERE ProductID = product_id AND YEAR(Date) = year
);
